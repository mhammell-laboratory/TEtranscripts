...
def _render_report(
    ...
    r_version,
    fixture_name,
    minimum_padj_spearman,
    minimum_significant_jaccard,
):
    ...
    lines = [
        ...
        "`%s` with three treatment and three control replicates." % fixture_name,
        "",
    ...
    if padj_spearman < minimum_padj_spearman:
        failures.append(
            "adjusted-p-value rank correlation is below %.2f"
            % minimum_padj_spearman
        )
    if sig_jaccard < minimum_significant_jaccard:
        failures.append(
            "significant-call Jaccard overlap is below %.2f"
            % minimum_significant_jaccard
        )
    ...
    if failures:
        ...
        lines.extend(
            (
                ...
                "- PASS: adjusted-p-value rank correlation >= %.2f"
                % minimum_padj_spearman,
                "- PASS: significant-call Jaccard overlap >= %.2f"
                % minimum_significant_jaccard,
            )
        )
    return "\n".join(lines) + "\n", failures
...
def main():
    ...
    parser.add_argument("--min-padj-spearman", type=float, default=0.85)
    parser.add_argument("--min-significant-jaccard", type=float, default=0.60)
    args = parser.parse_args()

    for option, value in (
        ("--min-padj-spearman", args.min_padj_spearman),
        ("--min-significant-jaccard", args.min_significant_jaccard),
    ):
        if not 0.0 <= value <= 1.0:
            parser.error("%s must be between 0 and 1" % option)
    ...
    report, failures = _render_report(
        ...
        r_version,
        os.path.basename(counts),
        args.min_padj_spearman,
        args.min_significant_jaccard,
    )
